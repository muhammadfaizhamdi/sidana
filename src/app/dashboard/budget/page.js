...
export default function BudgetPage() {
  ...
  if (isColdStart) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] max-w-lg mx-auto text-center px-4">
        <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-full flex items-center justify-center mb-6 shadow-inner transition-colors">
          <PieChart size={40} />
        </div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed transition-colors">
          Sistem belum mendeteksi riwayat pemasukan Anda. Untuk membuat batas aman <span className="font-bold text-indigo-600 dark:text-indigo-400">50/30/20</span> secara otomatis, berapa target atau perkiraan pemasukan bulanan Anda?
        </p>
        <form onSubmit={handleSaveTargetIncome} className="w-full bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl shadow-slate-100/50 dark:shadow-none transition-colors duration-500">
          ...
          <button type="submit" className="w-full bg-indigo-600 text-white font-bold py-4 rounded-2xl hover:bg-indigo-700 active:scale-[0.98] transition-all flex justify-center items-center gap-2 text-lg shadow-lg shadow-indigo-200 dark:shadow-none">
  ...
  const renderBudgetGroup = (groupType, title, colorClass, bgClass, maxLimit) => {
    ...
    // Penyesuaian class warna gelap dinamis
      ...
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col mb-6 hover:shadow-md transition-all duration-500">
        <header className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-6 pb-4 border-b border-slate-100 dark:border-slate-700/50">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Batas aman porsi Anda: <span className={`font-bold ${colorClass} ${darkColorClass}`}>{formatRupiah(maxLimit)}</span></p>
          </div>
          <div className="sm:text-right">
            ...
            <p className={`text-2xl font-extrabold ${isOverBudget ? 'text-rose-600 dark:text-rose-400' : 'text-slate-900 dark:text-white'}`}>{formatRupiah(totalAllocated)}</p>
          </div>
        </header>
              ...
              <div key={budget.id} className="group relative">
                <div className="flex justify-between items-end mb-2.5">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Terpakai {formatRupiah(spent)} dari <span className="font-bold text-slate-700 dark:text-slate-300">{formatRupiah(limit)}</span></p>
                  </div>
                  <button onClick={() => openEditModal(budget)} className="p-2 text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 rounded-xl transition-all sm:opacity-0 group-hover:opacity-100">
                    <Edit2 size={18} />
                  </button>
                </div>
