...
export default function WishlistPage() {
  ...
  const getRemainingMonths = (targetDate) => {
    ...
    const diff = (target.getFullYear() - new Date().getFullYear()) * 12 + (target.getMonth() - new Date().getMonth());
  ...
  const handleSaveWishlist = async (e) => {
    ...
    setIsAddModalOpen(false); setNewWishlist({ title: '', target_amount: '', target_date: '' }); setEditingId(null); fetchData();
  };
  ...
  const handleDeleteClick = async (id) => {
    ...
    await fetch(`/api/wishlist?id=${id}`, { method: 'DELETE' }); fetchData();
  };
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {wishlists.map(wish => {
          ...
          const recommendedMonthly = remainingAmount / getRemainingMonths(wish.target_date);
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1 text-xs text-slate-400 dark:text-slate-500 font-medium mt-0.5"><Calendar size={12} /><span>{new Date(wish.target_date).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })}</span></div>
                  </div>
                </div>
                
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity -mr-2">
                  <button onClick={() => handleEditClick(wish)} className="p-1.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 rounded-lg transition-colors"><Edit2 size={16} /></button>
                  <button onClick={() => handleDeleteClick(wish.id)} className="p-1.5 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/20 rounded-lg transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>

              <div className="mb-4"><p className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{formatMoney(target)}</p></div>

              <div className="mb-6">
                <div className="flex justify-between text-sm mb-1.5">
                  <span className="font-medium text-slate-500 dark:text-slate-400">Terkumpul {formatMoney(collected)}</span>
                  ...
                  <>
                    {isWarning ? (
                      ...
                      <div className="flex items-center justify-between text-sm mb-4 bg-slate-50 dark:bg-slate-700/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                        <span className="text-slate-500 dark:text-slate-400 font-medium">Saran setoran:</span><span className="font-bold text-slate-900 dark:text-white">{formatMoney(recommendedMonthly)}</span>
                      </div>
                    )}
                    <button onClick={() => openDepositModal(wish, isWarning ? safeLimit20Percent : recommendedMonthly)} className="w-full py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl shadow-sm hover:bg-indigo-700 active:scale-[0.98] transition-all flex justify-center items-center gap-2"><Wallet size={16} /> Setor Tabungan</button>
                  </>
                ) : (
                  <div className="w-full py-2.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-sm font-bold rounded-xl border border-emerald-100 dark:border-emerald-500/20 flex justify-center items-center gap-2"><CheckCircle2 size={16} /> Target Tercapai!</div>
                )}
              </div>
            </div>
          );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSaveWishlist} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white font-bold py-3.5 mt-2 rounded-xl shadow-sm hover:bg-indigo-700 active:scale-[0.98] transition-all">{editingId ? 'Simpan Perubahan' : 'Buat Target'}</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-sm p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleDeposit}>
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white font-bold py-3.5 rounded-xl shadow-sm hover:bg-indigo-700 active:scale-[0.98] transition-all flex justify-center items-center gap-2"><Wallet size={18} /> Pindahkan Dana</button>
            </form>
          </div>
        </div>
      )}
