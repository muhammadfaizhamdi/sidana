    ...
    <section className="space-y-6">
      ...
      <div className="relative w-full md:w-1/2 mb-2">
        ...
        <input 
          ...
          className="w-full pl-11 pr-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-600 outline-none transition-all text-slate-700 dark:text-white font-medium shadow-sm"
        />
      </div>
            ...
            {filteredTransactions.map(tx => (
              <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                ...
                <td className="p-4 text-sm text-slate-600 dark:text-slate-300">
                  <span className="bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-lg text-xs font-medium whitespace-nowrap transition-colors">
                    {tx.category}
                  </span>
                </td>
                ...
                <td className="p-4 text-center">
                  <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEditClick(tx)} className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors p-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-500/20" title="Edit Data">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleDelete(tx.id)} className="text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-rose-400 transition-colors p-2 rounded-lg hover:bg-red-50 dark:hover:bg-rose-500/20" title="Hapus Data">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
